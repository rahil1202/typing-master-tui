...
const PARAGRAPHS: ParagraphEntry[] = [
  { id: "p1", level: "beginner", flavor: "standard", text: "Every day you type, your fingers learn a little more. Slow and accurate practice builds strong habits that make speed feel natural." },
  { id: "p2", level: "beginner", flavor: "standard", text: "Good posture keeps your hands relaxed and your mind calm. When you breathe steadily, your rhythm improves and mistakes drop." },
  { id: "p3", level: "beginner", flavor: "standard", text: "A focused session is better than a rushed session. Watch each word, trust the rhythm, and let your accuracy guide your pace." },
  { id: "p4", level: "beginner", flavor: "story", text: "Mina opened the workshop door, found the note on the desk, and copied the first line before the morning bell rang." },
  { id: "p5", level: "intermediate", flavor: "standard", text: "Consistency wins long races. Instead of sudden bursts, maintain a stable cadence and recover quickly when an error appears." },
  { id: "p6", level: "intermediate", flavor: "standard", text: "Strong typists read slightly ahead while finishing the current word. This small preview reduces hesitation and smooths transitions." },
  { id: "p7", level: "intermediate", flavor: "standard", text: "When punctuation and numbers appear, keep your tempo under control. Precision on complex tokens prevents large accuracy penalties." },
  { id: "p8", level: "intermediate", flavor: "story", text: "The courier ran across the market, hid the message under a crate, and typed the checkpoint code before anyone noticed." },
  { id: "p9", level: "intermediate", flavor: "story", text: "On the training ship, the rookie copied each command from the captain and watched the dashboard settle into green." },
  { id: "p10", level: "advanced", flavor: "standard", text: "Under competitive pressure, elite performance depends on deliberate control, not panic. Strategic pacing preserves clarity and throughput." },
  { id: "p11", level: "advanced", flavor: "standard", text: "Technical writing contains symbols, mixed casing, and dense vocabulary. Efficient finger travel and disciplined correction maintain momentum." },
  { id: "p12", level: "advanced", flavor: "story", text: "By midnight the archive vault had opened, the alarms had softened, and Kara still needed to type the final sequence from memory." },
  { id: "p13", level: "advanced", flavor: "story", text: "The rescue drone hovered above the canyon while the navigator entered coordinates, weather notes, and fallback routes without a pause." },
  { id: "p14", level: "expert", flavor: "standard", text: "Sustained high-speed typing requires biomechanical efficiency, cognitive anticipation, and meticulous error recovery across variable complexity." },
  { id: "p15", level: "expert", flavor: "standard", text: "Mastery emerges when execution stays coherent under load: capitalization, punctuation, and numerics remain precise while tempo remains stable." },
  { id: "p16", level: "expert", flavor: "story", text: "The station lost external comms at 03:17, yet the operator still logged every signal, checksum, and route update before dawn." },
  { id: "p17", level: "expert", flavor: "story", text: "Inside the storm shelter, the engineer balanced equations, warning codes, and crew notes while the walls shook with each impact." }
];
...
export function getDifficultyConfig(level: Difficulty): DifficultyConfig {
  const map: Record<Difficulty, DifficultyConfig> = {
    beginner: { label: "Beginner", wordCount: 22, seedOffset: 11, loadingTicks: 22, custom: { punctuation: false, numbers: false, caseSensitive: false } },
    intermediate: { label: "Intermediate", wordCount: 35, seedOffset: 41, loadingTicks: 28, custom: { punctuation: true, numbers: false, caseSensitive: false } },
    advanced: { label: "Advanced", wordCount: 50, seedOffset: 79, loadingTicks: 34, custom: { punctuation: true, numbers: true, caseSensitive: true } },
    expert: { label: "Expert", wordCount: 70, seedOffset: 131, loadingTicks: 40, custom: { punctuation: true, numbers: true, caseSensitive: true } }
  };
  return map[level];
}
...
export function makeParagraphTest(
  ...
  while (wordCountOf(out) < targetWords) {
    s = nextSeed(s);
    out += ` ${eligible[s % eligible.length].text}`;
  }
...
export function buildCustomModeText(
  ...
  if (mode === "numbers") {
    return { text: buildNumericSequence(targetWords, seed), textId: `custom-numbers-${level}-${seed}`, label: "Numbers" };
  }
  if (mode === "uppercase-only") {
    return {
      text: buildWordSequence(targetWords, seed, WORDS, (word, index) => index % 7 === 0 ? `${word.toUpperCase()}!` : word.toUpperCase()),
      textId: `custom-uppercase-${level}-${seed}`,
      label: "Uppercase Only"
    };
  }
  if (mode === "lowercase-only") {
    return {
      text: buildWordSequence(targetWords, seed, WORDS, (word) => word.toLowerCase()),
      textId: `custom-lowercase-${level}-${seed}`,
      label: "Lowercase Only"
    };
  }
  if (mode === "combined") {
    return { text: buildCombinedSequence(targetWords, seed, false), textId: `custom-combined-${level}-${seed}`, label: "Combined" };
  }
  return {
    text: buildCombinedSequence(targetWords + 8, seed, true),
    textId: `custom-hard-combined-${level}-${seed}`,
    label: "Hard Combined + Symbols"
  };
}
...
function buildWordSequence(targetWords: number, seed: number, pool: string[], transform: (word: string, index: number) => string): string {
  let s = seed;
  const out: string[] = [];
  for (let i = 0; i < targetWords; i++) {
    s = nextSeed(s);
...
function buildCombinedSequence(targetWords: number, seed: number, withSymbols: boolean): string {
  ...
  for (let i = 0; i < targetWords; i++) {
    ...
    if (withSymbols && i % 4 === 0) out.push(SYMBOL_TOKENS[s % SYMBOL_TOKENS.length]);
    else if (i % 5 === 0) out.push(word.toUpperCase());
    else if (i % 3 === 0) out.push(`${capitalize(word)}${numeric.replace(/[^0-9]/g, "").slice(0, 2) || "7"}`);
    else if (i % 7 === 0) out.push(numeric);
    else out.push(word.toLowerCase());
  }
  return out.join(" ");
}
